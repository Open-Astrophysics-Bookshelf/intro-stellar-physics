...
Here we have used the shorthand $\beta = (\kB T)^{-1}$. Written in this fashion, it is clear that for $n\gg 1$, the sum diverges, since the individual terms go as $n^{2}e^{-\beta\ERy}$ as $n\to\infty$. In practice, this isn't a problem, as there is an upper limit on $n$. The mean distance of the electron is $\approx \abohr n^{2}$, where $\abohr = \val{\sci{5.29}{-11}}{\cm}$ is the Bohr radius. As a result, each atom takes up a volume $\approx \abohr^{3}n^{6}$; if we want the atoms to not overlap, then the volume per atom must be at least twice this. Thus for $N$ atoms, the number per volume (denoted by $\xi$ here to avoid confusion) must be
\[
\xi = \frac{N}{V} < \frac{N}{N\cdot 2\abohr^{3}n^{6}}.
\]
Thus the maximum level is $n < (2\abohr^{3}\xi)^{-1/6}$.

You might worry that we have been sloppy in defining this cutoff. The terms in the partition function are quite small, however, until $n$ becomes quite large. For example, at $T=\val{10^{4}}{\K}$,
\begin{center}
\begin{tabular}{rrr}
...
\end{tabular}
\end{center}
For a typical A-star photospheric density $\xi\sim \val{10^{15}}{\cm^{-3}}$, the density cutoff is $n \lesssim 35$; regardless of the precise value of the cutoff, it is reasonably accurate to set the sum to unity, so that $Q\approx 4e^{\beta\ERy}$.
\end{sidebar}
