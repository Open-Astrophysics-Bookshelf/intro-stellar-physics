...
\marginnote[\baselineskip]{\colorbox{yellow}{\textbf{Intensity is linear in $\mu$}}}
...
\newthought{We have our first result!} For a steady-state gray atmosphere in local thermal equilibrium, the total flux $F = \int F_{\nu}\,\dif\nu$ is constant. The radiative energy is just passing through. We know that outside the star, the flux is $F = \sigmaSB\,\Teff^{4}$, so we can write the intensity as
\[
	I_{\nu}(\mu,\tau) = \frac{c}{4\pi} U_{\nu}(\tau) + \frac{3\mu}{4\pi}\sigmaSB\Teff^{4}.
\]
To solve for $U_{\nu}(\tau)$, multiply eq.~(\ref{e.transfer-gray}) by $\mu$ and integrate over all frequencies and angles:
\[
