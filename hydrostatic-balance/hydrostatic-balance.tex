...
\subsection{Constant gravity}

Let's start with a simple case: a thin fluid layer, over which gravity is approximately constant. This is a good approximations for Earth's atmosphere or ocean. Exercise~\ref{ex.pressure-increase-ocean} examines the pressure increase in an incompressible (density is fixed) fluid. This is a bad approximation for a star, but a good one for Earth's oceans: the density of sea water increases by less than 5\% between the surface and ocean floor.

\begin{exercisebox}[Pressure increase in the ocean]
\label{ex.pressure-increase-ocean}
Seawater has a density $\rho = \val{10^{3}}{\kilo\gram\usk\meter^{-3}}$. Solve eq.~(\ref{e.hydrostatic-equilibrium-g}) to get an equation for the pressure as a function of depth in the ocean. How deep would you need to dive for the pressure to increase by $\val{1}{\unitstyle{atm}} = \val{\sci{1.013}{5}}{\unitstyle{Pa}}$? Does this agree with your experience?
\end{exercisebox}
...
We can write equation~(\ref{e.hydrostatic-equilibrium-g}) in the form of an integral:
\[ \int_{P_{0}}^{P_(z)} \dif P = -g\int_{0}^{z} \rho\,\dif z. \]
Consider a cylinder of cross-section $\Delta A$ that extends from $0$ to $z$.  The mass of that cylinder is
...
that is, the difference in pressure is the weight per unit area of our column.  Let's apply this to our atmosphere:
...
\end{equation}
Some examples may make this clearer. Suppose we have $n$ molecules of molecular hydrogen ($\mathrm{H}_{2}$). The mass of this sample is $\approx \val{2}{\mb}\times n$, since each molecule has 2 nucleons. The total number of particles in our sample is $n$, so
\[
	\mu(\mathrm{H_{2}}) = \frac{2\mb \times n}{n\mb} = 2.
\]
Now suppose we raise the temperature and dissociate those molecules into individual atoms. The mass of a sample of $n$ atoms is $\val{1}{\amu}\times n$, so $\mu = 1$. Let's raise the temperature further, so that the gas ionizes into electrons and nuclei (protons). This is a bit trickier.  The electrons contribute negligibly to the mass, so if we had $n$ atoms, the mass is still $\val{1}{\amu}\times n$. The total number of particles has \emph{doubled}, however, since for each atom there are now 2 particles (electron and nucleus). The mean molecular weight is therefore
\[
...
To recap, we now have two differential equations, (\ref{e.mass-continuity}) and (\ref{e.hydrostatic-equilibrium}), that describe the structure of a star. These equations are for the pressure $P(r)$ and density $\rho(r)$ in the star. We can't solve these equations, however, because we don't yet have a relation between $P$ and $\rho$. For example, the ideal gas equation of state relates $P$ and $\rho$ via a temperature $T$, so at a minimum we need an equation for $T(r)$. 
We'll defer the derivation of additional stellar structures a bit, and explore what we can learn from just these two equations. To proceed, we need to prescribe $\rho(r)$; let's use a constant density and see where that leads.
...
\]
Notice that this expression contains the combination $\sqrt{R^{3}/M}\propto 1/\sqrt{\bar{\rho}}$, where $\bar{\rho}$ is the mean density:
...
\end{equation}
Here $\gamma$ is the adiabatic index: for an ideal monatomic gas, $\gamma = 5/3$. The star can therefore respond to pressure changes on a \newterm{sound-crossing time} $\tau_{\mathrm{sc}} = R/\bar{c}_{s}$. If we use the central temperature for a constant density star (exercise~\ref{ex.constant-density-star}) to get an estimate for the mean $c_{s}$,
\[
	\tau_{\mathrm{sc}} \equiv \frac{R}{\bar{c}_{s}} \approx R\left(\frac{\mu\mb}{\gamma\kB T_{c}}\right)^{1/2} = R\left(\frac{2R}{\gamma GM}\right)^{1/2}
		= \left(\frac{3}{\sqrt{10\pi}}\right)\frac{1}{\sqrt{G\bar{\rho}}}.
\]
Both the sound-crossing time, $\tau_{\mathrm{sc}}$, and the free-fall time, $\tau_{\mathrm{ff}}$, are of the order of the dynamical timescale $1/\sqrt{G\bar{\rho}}$.

We have another way of viewing hydrostatic equilibrium: the star is able to remain in balance because the time for pressure disturbances to propagate is comparable to the time for large-scale motions of the fluid
\[ \tau_{\mathrm{sc}} \sim \tau_{\mathrm{ff}} \sim \tau_{\mathrm{dyn}}. \]
For the sun, $\bar{\rho} = \val{1400}{\kilo\gram\usk\meter^{-3}}$; this is just a bit denser than you\sidenote{The density of an adult human body, which is about 60\% water, is close to $\val{10^{3}}{\kilo\gram\usk\meter^{-3}}$; most of us are close to neutral buoyancy in water.}. The dynamical timescale for the sun is thus about one hour.
...
\item
If the expansion is adiabatic, then the new pressure obeys a relation $P' V'^{\gamma} = PV^{\gamma}$ (Box \ref{b.sound-speed}). For our shell of mass $m$, show that this implies that $P'\rho'^{-\gamma} = P\rho^{-\gamma}$, and thus find $P'$ in terms of $P$, $\delta R/R$, and $\gamma$, to lowest order in $\delta R/R$.
...
\caption[Star formation in Rho Ophiuchi]{\label{f.rho-ophiuchi} Star forming regions in the Rho Ophiuchi cloud. \imgcred\ NASA, JPL-Caltech, WISE.}
\end{marginfigure}
