...
What would happen if the star suddenly fell out of hydrostatic equilibrium? To explore this, suppose that we could turn off the pressure. If we did, then the gas in the star would free-fall towards the center and the star would implode. How long would this take? If the gas is free-falling, we merely need to integrate the equation of motion for a particle free-falling from the surface to the center. There is a simpler method, however, using Kepler's law. Start with a circular orbit and deform it while keeping the sun at one focus, as shown in Fig.~\ref{f.fall-to-center}.
...
\nonumber
	K &=& \frac{1}{2}\sum_{i=1}^{N}\bvec{p}_{i}\vdot\DDt{\bvec{r}_{i}}\\
		&=& \frac{1}{2}\left[\DDt{}\underbrace{\left(\sum_{i=1}^{N}\bvec{p}_{i}\vdot\bvec{r}_{i}\right)}_{\equiv G} - \sum_{i=1}^{N}\bvec{r}_{i}\vdot\underbrace{\DDt{\bvec{p}_{i}}}_{=\bvec{F}_{i}}\right].
\label{e.expand-K}
\end{eqnarray}
...
\end{equation}
This timescale is called the \newterm{Kelvin-Helmholtz timescale}.  Since $t_{\mathrm{KH}} \gg t_{\mathrm{dyn}} = (G\bar{\rho})^{-1/2}$ the star is, to an excellent approximation, in hydrostatic equilibrium throughout the whole contraction.
