...
Now let's . Do you remember the random walk? If not, review Appendix C.2 of the \href{http://www.pa.msu.edu/~ebrown/docs/AST208-notes.pdf}{notes from AST 208}. The photon moves at speed $c$, but it can only go one step, of length $\lambda$, at a time. Before each step, the photon flips a coin: heads to go right, tails to go left. On average, the photon doesn't get anywhere; but if it takes enough hops, there is some probability for it to reach the edge of the star. The plot below shows the distribution for increasingly larger numbers of steps $n$. If there is an edge, say at the red vertical lines, then after some number of steps the photon will reach the edge and escape.
