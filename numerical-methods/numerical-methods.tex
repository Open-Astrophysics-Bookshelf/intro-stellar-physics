...
\end{array}\right].
\]
This is a \newterm{tridiagonal} matrix equation\sidenote{Blank entries are zero}, and can be efficiently solved as follows.
...
\item At this point we have the solution for $k_{N}$: $k_{N}=d'_{N}$. Starting with row $N-1$, we then work backwards zeroing out the $c'_{i}$: if row $i$ and $i+1$ are\marginnote{When we set $c'_{i+1}$ set to zero, we can then equate $k_{i+1}$ with its corresponding entry on the RHS of the matrix equation}
\[
...
\end{enumerate}
Thus, after iterating over the rows twice---forwards to eliminate the $a_{i}$, backwards to eliminate the $c_{i}$---we have the derivatives $k_{i}$. If we then wish to interpolate a value $y(x)$, we simply need to find $i$ such that $x_{i}< x < x_{i+1}$ and then use $x_{i,i+1}$, $y_{i,i+1}$, and $k_{i,i+1}$ in equation (\ref{e.spline}).
