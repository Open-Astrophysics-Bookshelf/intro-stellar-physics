...
We can estimate how the luminosity depends on stellar mass for stars that have a mostly radiative structure. As in exercise~\ref{ex.radiative-transfer-diffusion}, assume that $aT^{4}=aT_{c}^{4}(1-r/R)$. Use this in eq.~(\ref{e.temperature-radiative}); also assume a constant opacity $\kappa$, replace $\rho$ and $T_{c}$ with the average density (in terms of stellar mass $M$ and and radius $R$) and virial estimate of the central temperature, and replace the factor of $r^{2}$ with $R^{2}$. After some algebra, you should find that the luminosity $L$ depends on $M$ to some power.
\end{exercisebox}
