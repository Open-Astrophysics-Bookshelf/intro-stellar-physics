...
Experimentally, nuclei are on the order of femtometers\sidenote{Sometimes called a \emph{Fermi}} ($\val{1}{\fermi} = \val{10^{-15}}{\meter}$) in size. Like an atom, the nucleus also has excited states; typical energies for these are on the order of MeV ($\val{1}{\MeV} = \val{10^{6}}{\eV}$). \marginnote{Recall that an \newterm{electron volt} (eV) is the energy acquired by an electron being accelerated through a potential difference of 1 volt.} It therefore makes sense to use fm and MeV as our units of length and energy. In these units, the combination
...
\[	\alphaF = \frac{e^{2}}{4\pi\epsilon_{0}\hbar c} = (137)^{-1}, \]
again to three significant figures. From these two quantities, we see that
\[
...
The strong nuclear force differs from electromagnetism and gravity in that it is short-range: the interaction vanishes for distances $\gtrsim\val{2}{\fermi}$. It is weakly attractive for distances $\approx\val{1}{\fermi}$ and becomes strongly repulsive at short-range, $\ll\val{1}{\fermi}$. The nuclear force does not distinguish between neutrons and protons. Because the force is weakly attractive for separations $\val{1}{\fermi}\lesssim r\lesssim\val{2}{\fermi}$ and repulsive at shorter distances, in large nuclei there is a characteristic distance between nucleons that is a bit larger than $\val{1}{\fermi}$.

\begin{marginfigure}
\includegraphics[width=\linewidth]{nuclear-potential}
\caption{\label{f.nuclear-potential}Schematic of the nuclear potential for a deuteron (\hydrogen[2]). The binding energy of the deuteron is shown as a black dotted line.}
\end{marginfigure}
...
\newthought{Despite these complications, there are many properties of the nucleus that we can deduce.}
