...
\begin{eqnarray*}
	V[P(r+h),S(r)] &<& V[P(r+h),S(r+h)] \\
	&\approx& V[P(r+h),S(r)] + \tderiv{V}{S}{P}\DD{S}{r}.
\end{eqnarray*}
Here the notation $(\partial V/\partial S)_{P}$ means taking the derivative of $V$ with respect to $S$ while holding $P$ fixed. The condition for stability is therefore, after canceling common factors,
\begin{equation}\label{e.convective-stability}
 \tderiv{V}{S}{P}\frac{\dif S}{\dif r} > 0 .
\end{equation}
We can put this into a more useful form by changing variables from entropy $S$ to temperature $T$ via
\[
...
Now, $(\partial V/\partial T)_{P}$ is positive (gas expands on being heated) and so is $C_{P}$, so the condition for stability is satisfied wherever
