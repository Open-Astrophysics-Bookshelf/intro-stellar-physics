...
\]
You stand (at bottom in Fig.~\ref{f.intensity-conserved}) with a camera, the aperture of which has area $\Delta A_{\mathrm{obs.}}$, a distance $d$ ($d^{2}\gg \Delta A_{\mathrm{emit}}, \Delta A_{\mathrm{obs}}$) from your friend. You measure an intensity $I_{\lambda,\mathrm{obs}}$. Show that  $I_{\lambda,\mathrm{obs}} = I_{\lambda,\mathrm{emit}}$ by doing the following.
\begin{enumerate}
\item\label{ex.intensity-conserved-one} Calculate the incident energy that falls on your camera aperture $\Delta E_{\mathrm{obs}}$.
\item\label{ex.intensity-conserved-two} Find the solid angle $\Delta\Omega_{\mathrm{obs}}$ subtended by the rays entering the aperture.
\item Now use parts \ref{ex.intensity-conserved-one} and \ref{ex.intensity-conserved-two} to compute your intensity
\[	
	I_{\lambda,\mathrm{obs}} = \frac{\Delta E_{\mathrm{obs}}}{\Delta t\,\Delta A_{\mathrm{obs}}\,\Delta\lambda \,\Delta\Omega_{\mathrm{obs}}}.
\]
and show that this is the same as $I_{\lambda,\mathrm{emit}}$.
\end{enumerate}
\end{exercisebox}
...
\end{equation}
By changing variables to $x = hc/\lambda\kB T$, we can write this integral as\marginnote{The integral over $x$ can be converted into $3!\times\zeta(4)$, where $\zeta$ is the Riemann zeta function; for further information, consult a text on mathematical methods in physics.}
\[
...
Because equation~(\ref{e.apparent-magnitude}) is a flux ratio, the absolute scale is not defined. Conversion from a magnitude to physical units, such as $\watt\usk\meter^{-2}\usk\nano\meter^{-1}$, is unfortunately difficult and not straightforward. Instead, the magnitude system is calibrated to specific stars.  The zero-point is set so that Vega has $U = B = V = \ldots = 0$\sidenote{The modern value is $V(\textrm{Vega}) = 0.026 \pm 0.008$; \citet{Bohlin2004Hubble-Space-Te}.}. 
