...
\begin{exercisebox}
Explain, without using mathematical expressions, why $F_{\lambda}=0$ if the emission is thermal.
\end{exercisebox}

Although the net flux is zero, if we just want the radiation escaping from our cavity, we only want to integrate over the angles $0\le\theta\le\pi/2$. If we do this, then our \emph{outward-going} flux is
